...
//    | _ -> []
//
//    let write = fun path tasks ->
//        let names = function
//        | Some (cmds:'a list) ->
//            let _, res = List.fold (fun (i, acc) _ -> if i > 0 then (i - 1, i :: acc) else (i, acc)) (cmds.Length, []) cmds
//            res
...
//            
//            List.fold2
//                (fun _ name content -> File.WriteAllText(path name, content))
//                ()
//                names
//                (tasksToStrs tasks)
//        | _ -> ()
...
[<EntryPoint>]
let main _ =
    let CUSTOM: string * string * string -> string = Regex.Replace

    let CHC: string * string * string -> string = fun (x, _, _) -> x

    let from = "/home/andrew/smt/analysis-vampire/rin-gen/drop.smt2"

    let to' = "/home/andrew/smt/analysis-vampire/rin-gen/drop.out"

    let parser = Parser()
    
    let a = parser.ParseFile <| from
    
    let b = parser.ParseFile <| to'
    
//    CUSTOM |> Program.addDeclarations from to'
//
//    let cmds = Parser().ParseFile(to')
//
...
//    let cmds = Parser().ParseFile(from)
